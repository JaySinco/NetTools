...
http::http(const u_char *const start, const u_char *&end, const protocol *prev)
{
...
void http::to_bytes(std::vector<u_char> &bytes) const
{
...
bool http::link_to(const protocol &rhs) const { throw std::runtime_error("unimplemented method"); }

const http::detail &http::get_detail() const { return d; }
