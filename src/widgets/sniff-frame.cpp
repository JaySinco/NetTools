#include "sniff-frame.h"
#include "packet-listctrl.h"
#include "packet-propgrid.h"
#include "transport/port-table.h"
...
#define NOTIFY_CATCH                                                                   \
    ...
    {                                                                                  \
        ...
        GetEventHandler()->CallAfter(std::bind(&SniffFrame::notify_error, this, msg)); \
    }

SniffFrame::SniffFrame() : SniffFrame_g(nullptr)
{
    ...
    m_filter->SetFocus();
    std::thread(&SniffFrame::port_table_update_background, this, 10).detach();
    m_start->Bind(wxEVT_BUTTON, &SniffFrame::on_sniff_start, this);
    m_stop->Bind(wxEVT_BUTTON, &SniffFrame::on_sniff_stop, this);
    m_clear->Bind(wxEVT_BUTTON, &SniffFrame::on_sniff_clear, this);
    m_list->Bind(wxEVT_LIST_ITEM_SELECTED, &SniffFrame::on_packet_selected, this);
    m_list->Bind(wxEVT_LIST_COL_CLICK, &SniffFrame::on_list_col_clicked, this);
    m_filter->Bind(wxEVT_TEXT_ENTER, &SniffFrame::on_filter_changed, this);
}

void SniffFrame::on_sniff_start(wxCommandEvent &event)
{
    m_start->Disable();
    m_stop->Enable();
    sniff_should_stop = false;
    const adaptor &apt = adaptor::all().at(m_adaptor->GetSelection());
    std::thread(&SniffFrame::sniff_background, this, apt, "", 500).detach();
}

void SniffFrame::on_sniff_stop(wxCommandEvent &event) { sniff_should_stop = true; }

void SniffFrame::on_sniff_clear(wxCommandEvent &event)
{
    m_list->clear();
    m_prop->Clear();
    idx_list.clear();
...
void SniffFrame::on_packet_selected(wxListEvent &event)
{
    m_prop->show_packet(pac_list.at(idx_list.at(event.m_itemIndex)));
}

void SniffFrame::on_list_col_clicked(wxListEvent &event)
{
...
void SniffFrame::on_filter_changed(wxCommandEvent &event)
{
    ...
    for (size_t i = 0; i < pac_list.size(); ++i) {
        if (!filter_ || filter_->test(pac_list[i].to_json())) {
...
void SniffFrame::sniff_background(const adaptor &apt, const std::string &filter, int update_freq_ms)
{
    NOTIFY_TRY
    pcap_t *handle = transport::open_adaptor(apt, update_freq_ms);
    std::shared_ptr<void> handle_guard(nullptr, [&](void *) {
        pcap_close(handle);
        GetEventHandler()->CallAfter(std::bind(&SniffFrame::sniff_stopped, this));
        ...
        if (duration.count() >= update_freq_ms) {
            if (data.size() > 0) {
                GetEventHandler()->CallAfter(
                    std::bind(&SniffFrame::sniff_recv, this, std::move(data)));
                data.clear();
            }
            start_tm = now;
        }
...
void SniffFrame::port_table_update_background(int update_freq_ms)
{
    while (true) {
        port_table::update();
...
void SniffFrame::sniff_recv(std::vector<packet> data)
{
    bool need_refresh = false;
    for (auto &pac : data) {
        pac_list.push_back(std::move(pac));
        if (!filter_ || filter_->test(pac_list.back().to_json())) {
            need_refresh = true;
...
void SniffFrame::sniff_stopped()
{
    m_stop->Disable();
    m_start->Enable();
}

void SniffFrame::notify_error(const std::string &msg)
{
    wxMessageDialog diag(this, msg, "Error", wxOK | wxICON_ERROR);
    diag.ShowModal();
}

void SniffFrame::update_status_bar()
{
