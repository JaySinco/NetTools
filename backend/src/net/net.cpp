...
pcap_t *open_target_adaptor(const ip4_addr &ip, bool exact_match, adapter_info &apt_info)
{
    ...
    if (apt_info.name.size() == 0) {
        throw std::runtime_error(
            fmt::format("no adapter found in same local network with {}", to_string(ip)));
    }
    pcap_t *adhandle;
    char errbuf[PCAP_ERRBUF_SIZE];
    if (!(adhandle = pcap_open(apt_info.name.c_str(), 65536, PCAP_OPENFLAG_PROMISCUOUS, 1000, NULL,
                               errbuf))) {
...
adapter_info adapter_info::select_ip(const ip4_addr &subnet_ip, bool exact_match)
{
    ...
    while (pAdapter) {
        ...
        bool subnet_cond = (!exact_match && apt_mask != PLACEHOLDER_IPv4_ADDR &&
                            (apt_ip.same_subnet(subnet_ip, apt_mask)));
...
adapter_info adapter_info::select_auto() { return select_ip(PLACEHOLDER_IPv4_ADDR, false); }
                ...
                int timeout_ms, const packet_loop_callback &cb)
{
    ...
    while ((res = pcap_next_ex(adhandle, &pkthdr, &pkt_data)) >= 0) {
        if (timeout_ms > 0) {
            ...
            auto duration =
                std::chrono::duration_cast<std::chrono::milliseconds>(now_tm - start_tm);
...
std::ostream &operator<<(std::ostream &out, const pcap_if_t *dev)
{
    ...
    for (pcap_addr_t *a = dev->addresses; a; a = a->next) {
        ...
        if (a->addr) {
            ...
            auto waddr = reinterpret_cast<const sockaddr_in *>(a->addr)->sin_addr;
            if (get_detail && apt_info.ip == waddr && apt_info.gateway != PLACEHOLDER_IPv4_ADDR) {
...
int ip2mac(pcap_t *adhandle, const adapter_info &apt_info, const ip4_addr &ip, eth_addr &mac,
           int timeout_ms, bool use_cache)
{
    auto start_tm = std::chrono::system_clock::now();
    static std::map<ip4_addr, ip_mac_cache> ip_mac_map;
    if (use_cache && ip_mac_map.count(ip) > 0) {
        auto passed_sec =
            std::chrono::duration_cast<std::chrono::seconds>(start_tm - ip_mac_map[ip].tm);
    ...
    int rtn = packet_loop(adhandle, start_tm, timeout_ms,
                          [&](pcap_pkthdr *pkthdr, const ethernet_header *eh) {
         ...
         int timeout_ms, long &cost_ms, ip_header &ih_recv, _icmp_error_detail &d_err)
{
    ...
    if (!is_local) {
        VLOG(1) << "nonlocal target ip, send icmp to gateway instread of "
                   "broadcasting";
        if (ip2mac(adhandle, apt_info, apt_info.gateway, dest_mac, 5000) != NTLS_SUCC) {
            throw std::runtime_error(fmt::format("can't resolve mac address of gateway {}",
                                                 to_string(apt_info.gateway)));
        }
    }
    timeval send_tv;
    ...
    int rtn = packet_loop(
        adhandle, start_tm, timeout_ms, [&](pcap_pkthdr *pkthdr, const ethernet_header *eh) {
            if (eh->d.dea == apt_info.mac && ntohs(eh->d.eth_type) == ETHERNET_TYPE_IPv4) {
                ...
                if (ih->d.dia == apt_info.ip && ih->d.proto == IPv4_TYPE_ICMP) {
                    ...
                    if (mh->d.type == ICMP_TYPE_PING_REPLY && mh->d.id == ping_data.id &&
                        mh->d.sn == ping_data.sn) {
