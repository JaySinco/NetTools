...
bool MessageHandler::OnQuery(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame,
                             int64 query_id, const CefString &request, bool persistent,
                             CefRefPtr<Callback> callback)
{
    std::wstring request_ws = request;
    auto comma_i = request_ws.find_first_of(L':');
    if (comma_i == std::wstring::npos || comma_i == request_ws.size() - 1 ||
        request_ws.at(comma_i + 1) != L' ') {
    ...
    if (header == L"get_resdir") {
        ...
        std::thread([=] {
            ...
            if (ShellExecuteExW(&execInfo)) {
                ...
                callback->Failure(NTLS_FAILED,
                                  fmt::format("failed to execute pack script: {}", GetLastError()));
            }
        }).detach();
        return true;
    }
